...
async function main() {
    try {
        ...
        for (const opinion of res.rows) {
            [addOps, geo_id] = await processOpinionClusterById(opinion.id, client);
            ops.push(...addOps);
            ...
            if ((ops.length > 6750) && (pushNeeded)) {
                ...
                fs.writeFileSync(`out_ops/publish_${ops.length}_op_opinionCluster_segment_${outId}.txt`, outputText);
                
                // Once you have the ops you can publish them to IPFS and your space.
                txHash = await publish({
                    spaceId,
                    author: walletAddress,
                    editName: `Publish operation opinionCluster segment ${outId}`,
                    ops: ops, // An edit accepts an array of Ops
                });
                
                console.log(ops.length)
                console.log("Your transaction hash is:", txHash);
                console.log(`Publish operation opinionCluster segment ${outId}`)
                pushNeeded = false
                ops.length = 0;
                clearFlag = clearFlag + 1
                if (clearFlag == 50) {
                    console.log("CLEAR EDITED")
