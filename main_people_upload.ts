...
async function main() {
    try {
        ...
        for (const person of res.rows) {
            [addOps, geo_id] = await processPerson(person.person_id, client);
            ops.push(...addOps)
            
            //addOps = await processOpinionCluster(person.person_id, client);
            //ops.push(...addOps)
            
            pushNeeded = true;
            outId = person.person_id;

            if ((ops.length > 6500) && (pushNeeded)) {
                // Once you have the ops you can publish them to IPFS and your space.
                txHash = await publish({
                    spaceId,
                    author: walletAddress,
                    editName: `Publish operation segment ${outId}`,
                    ops: ops, // An edit accepts an array of Ops
                });
                
                console.log(ops.length)
                console.log("Your transaction hash is:", txHash);
                console.log(`Publish operation segment ${outId}`)
                pushNeeded = false
                await clearEdited(client)
                // Convert operations to a readable JSON format
                outputText = JSON.stringify(ops, null, 2);
                // Write to a text file
                fs.writeFileSync(`out_ops/publish_op_segment_${outId}.txt`, outputText);
                ops.length = 0;
            }
        }
