...
export async function processPerson(inputId, client): string | null {
    try {
        //if (!client._connected) { // Assuming _connected is a property that indicates connection status
        ...
        for (const person of people) {
            ...
            if (!person.geo_id){
                ...
                if (person.date_dob !== null) {
                    ...
                    const dobTriple = Triple.make({
                            entityId: newGeoId,
                            attributeId: dobPropertyId,
                            value: {
                                type: "TIME",
                                value: dateOfBirth,
                                options: {
                ...
                if (person.date_dod !== null) {
                    ...
                    const dodTriple = Triple.make({
                            entityId: newGeoId,
                            attributeId: dodPropertyId,
                            value: {
                                type: "TIME",
                                value: dateOfDeath,
                                options: {
